...
export function LossFormComponent({ onSuccess }: { onSuccess: () => void }) {
  ...
  const onSubmit = async (data: LossFormValues) => {
    ...
    try {
      ...
      await db.transaction('rw', db.trades, db.ledger, async () => {
        ...
        // Negative entry for lost asset (uses proportional cost basis reduction)
        await db.ledger.add({
          tradeId: tradeId as number,
          accountId: parseInt(data.accountId),
          assetTicker: ticker,
          amount: -amount, // Negative amount for loss
          usdPriceAtTime: usdPrice, // Record current price for historical reference
        });
      });
      ...
      console.error('Failed to save loss transaction', error);
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="border rounded-lg p-3 bg-red-50 dark:bg-red-950/20">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Record losses from perp trading, liquidations, prediction losses, or fees. Uses proportional cost basis reduction.
        </p>
      ...
      <div>
        ...
        <Textarea 
          id="notes" 
          placeholder="e.g., Perp loss on ETH short, Liquidation, Prediction bet loss, Protocol fee..." 
          {...form.register("notes")} 
        />
      </div>
